...
interface CatalogViewProps {
  /** Set when rendered from `/catalog/[slug]` for stable SSR category context. */
  categorySlug?: string;
  /** Category id validated on the server for `/catalog/[slug]` routes. */
  initialCategoryId?: string;
}

export function CatalogView({
  categorySlug: categorySlugProp,
  initialCategoryId: initialCategoryIdProp,
}: CatalogViewProps = {}) {
  return (
...
function CatalogContent({
  ...
  const categoryFromLegacyUrl = searchParams.get('category');
  const categorySlugFromPath = getCategorySlugFromPathname(pathname);
  const categorySlug = categorySlugProp ?? categorySlugFromPath;
  const batchFromUrl = getCatalogBatchCount(searchParams);
  const catalogHrefKeyRef = useRef<string>('');

  const [selectedCategory, setSelectedCategory] = useState<string>(
    () => initialCategoryIdProp ?? 'all'
  );
  ...
  // Resolve category slug (route) or legacy ?category=id to selected category
  useEffect(() => {
    if (initialCategoryIdProp) {
      setSelectedCategory(initialCategoryIdProp);
      return;
    }

    if (categoryFromLegacyUrl && !categoriesLoading && categories.length > 0) {
      const legacy = categories.find((c) => c.id === categoryFromLegacyUrl);
      if (legacy?.slug) {
        const p = new URLSearchParams(searchParams.toString());
        p.delete('category');
        const qs = p.toString();
        const target = qs
          ? `${getCategoryCatalogPath(legacy)}?${qs}`
          : getCategoryCatalogPath(legacy);
        router.replace(target);
        return;
      }
      setSelectedCategory(categoryFromLegacyUrl);
      return;
    }

    if (categorySlug && !categoriesLoading && categories.length > 0) {
      const fromSlug = categories.find((c) => c.slug === categorySlug);
      if (fromSlug) {
        setSelectedCategory(fromSlug.id);
      }
      return;
    }
    ...
    try {
      ...
      if (!s?.startsWith(CATALOG_BASE_PATH)) return;
      const u = new URL(s, window.location.origin);
      const storedSlug = getCategorySlugFromPathname(u.pathname);
      if (storedSlug) {
        const fromStored = categories.find((c) => c.slug === storedSlug);
        if (fromStored) {
          setSelectedCategory(fromStored.id);
          return;
        }
      }
      const storedCategoryId = u.searchParams.get('category');
      if (storedCategoryId) {
  ...
  // Handle category selection — navigate to slug route for SEO-friendly URLs
  const handleCategorySelect = (categoryId: string) => {
    ...
    if (categoryId === 'all') {
      const qs = p.toString();
      router.push(qs ? `${CATALOG_BASE_PATH}?${qs}` : CATALOG_BASE_PATH);
      return;
    }
    ...
    if (category?.slug) {
      const qs = p.toString();
      const target = qs
        ? `${getCategoryCatalogPath(category)}?${qs}`
        : getCategoryCatalogPath(category);
      router.push(target);
      return;
    }
